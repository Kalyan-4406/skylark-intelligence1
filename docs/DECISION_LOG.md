...
I implemented an explicit dual-source strategy behind one canonical `BusinessDataSource` interface. A local-only workbook adapter supports fast, isolated development without API limits; production selects the Monday GraphQL adapter with `DATA_SOURCE=monday` and always queries both boards dynamically. Workbook files remain outside the production bundle, preserving the assignment's requirement that hosted answers come from Monday rather than static data.
